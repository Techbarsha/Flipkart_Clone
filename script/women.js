...
let getdataData = async () => {
  try {
    let res = await fetch("https://flipkart01.herokuapp.com/women");
...
div.append(img1,brandname,shortname,color,discountedprice,originalprice,discountpercent,rating);
         document.getElementById("right").append(div);

         div.onclick=()=>{
        localStorage.setItem("onedata",JSON.stringify(element._id))
        window.location.href="productPagewomen.html"
        console.log(element)
        }
     });
};
